...
class GameService {
  ...
  /// Get game questions by type
  Future<List<Map<String, dynamic>>> getGameQuestions(String gameType) async {
    try {
  ...
  Future<bool> canPlayGame(String gameId) async {
    try {
      ...
      final featuresAccess = userData['featuresAccess'] as Map<String, dynamic>?;
  ...
  Future<void> _updateUserProgressOnStart(String gameId) async {
    try {
      ...
      if (doc.exists) {
        ...
        final existingIndex = playedGames.indexWhere((g) => g['gameId'] == gameId);
